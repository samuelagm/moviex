...
func (cq *CommentQuery) QueryFilm() *MovieQuery {
...
func (cq *CommentQuery) WithFilm(opts ...func(*MovieQuery)) *CommentQuery {
	query := &MovieQuery{config: cq.config}
...
func (cq *CommentQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Comment, error) {
	...
	if query := cq.withFilm; query != nil {
		if err := cq.loadFilm(ctx, query, nodes, nil,
			func(n *Comment, e *Movie) { n.Edges.Film = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (cq *CommentQuery) loadFilm(ctx context.Context, query *MovieQuery, nodes []*Comment, init func(*Comment), assign func(*Comment, *Movie)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Comment)
	for i := range nodes {
		if nodes[i].movie_comments == nil {
			continue
		}
		fk := *nodes[i].movie_comments
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "movie_comments" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
